    ...
    <section>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {recipes.map((recipe, index) => (
          ...
          >
            <div className="relative bg-white/15 backdrop-blur-xl border border-white/25 rounded-2xl md:rounded-3xl overflow-hidden shadow-lg md:shadow-2xl shadow-cyan-500/5 hover:shadow-cyan-500/15 transition-all duration-500 group-hover:bg-white/20">
              <div
              ...
              >
                ...
                <div className="relative z-10 p-4 md:p-8">
                  <div className="flex items-center justify-between mb-3 md:mb-4">
                    <span
                      className={`text-xs font-semibold px-2 md:px-3 py-1 md:py-1.5 rounded-full ${
                        recipe.type === "Air Tawar"
                          ? "text-cyan-700 bg-cyan-100/90"
                          : "text-teal-700 bg-teal-100/90"
                      }`}
                    >
                      {recipe.type}
                    </span>
                  ...
                  <h3 className="font-bold text-slate-800 mb-3 md:mb-4 text-base md:text-xl group-hover:text-cyan-600 transition-colors duration-200 line-clamp-2">
                    {recipe.name}
                  </h3>
      ...
      {recipes.length === 0 && !isFavoritePage && (
        <div className="text-center py-16">
          <p className="text-slate-500">
            Ikan tidak ditemukan. Coba kata kunci lain.
          </p>
        </div>
      )}
    </section>
